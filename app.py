...
app = Flask(__name__)
app.secret_key = 'sua_chave_secreta'  # Certifique-se de definir uma chave secreta para sessões
CORS(app)

# Função de login para proteger o chatbot
def login_required(f):
    def wrapped_function(*args, **kwargs):  # Renomeei de 'wrap' para 'wrapped_function'
...
def login():
    if request.method == 'POST':
        ...
        # Valide as credenciais do usuário aqui (exemplo simples)
        if usuario == 'admin' and senha == 'senha123':
            session['usuario_id'] = usuario  # Guarda o ID do usuário na sessão
